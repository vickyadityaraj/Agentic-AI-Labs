...
Agentic AI Lab/
│
├── .env                                       # Environment configuration storing GROQ_API_KEY
...
├── college.db                                 # SQLite database generated by Experiment 1
├── Applied Agentic AI – Laboratory Experiments.md # Detailed documentation & report
...
*(Note: Experiments 2 and 3 use standard library math and collections modules, so external libraries like `numpy` are not strictly required, guaranteeing maximum portability.)*
...
The scripts automatically detect the API key in order of priority:
1. `.env` file containing: `GROQ_API_KEY=gsk_...`
2. Environment Variable: `GROQ_API_KEY`
3. Interactive Console Prompt (with hidden/masked entry)
